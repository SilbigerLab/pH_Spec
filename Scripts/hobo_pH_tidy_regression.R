...
hobo.ph.data$file.ID[hobo.ph.data$file.ID=='Data/HOBO_MX2501/hobo_logger/tidy_data/pH_SN195_20200303.csv']<- "SN195"
hobo.ph.data$file.ID[hobo.ph.data$file.ID=='Data/HOBO_MX2501/hobo_logger/tidy_data/pH_SN197_20200303.csv']<- "SN197"
#View(hobo.ph.data)
...
# graphing
modelR2<-function(long.data){
  m<-lm(pHmean~PST,long.data);
...
PlotTemp.long<-ggplot(data=long.data,aes(y=Temp_degC,x=PST,fill=file.ID))+ # basic plot
  ...
  theme(panel.border = element_blank(), panel.grid.major = element_blank(),
        panel.grid.minor = element_blank(), axis.line = element_line(colour = "black"))+
  geom_text(x = 11, y = 28, label=modelR2(long.data), parse = TRUE)
PlotTemp.long
...
# modeling
Reg.Model<-lm(Temp_degC~PST,data=long.data) # comparing temperature data between and among hobo's and sensitive thermometer
summary(Reg.Model)
Reg.Model

RegModel.time<-lm(pHmean~PST,data=spec.197) # comparing pH of SN197 to spec ~ time
summary(RegModel.time)
RegModel.time
RegModel.temp<-lm(pHmean~Temp_degC,data=spec.197) # comparing pH of SN197 to spec ~ temperature
...
PlotpH.TS<-ggplot(data=hobo.convert,aes(y=pH_TS,x=PST,fill=file.ID))+ # basic plot
  ...
  theme(panel.border = element_blank(), panel.grid.major = element_blank(),
        panel.grid.minor = element_blank(), axis.line = element_line(colour = "black"))+
  ggsave(paste0(output,"TotalScalepH_specHOBO_",date,".png"))+
